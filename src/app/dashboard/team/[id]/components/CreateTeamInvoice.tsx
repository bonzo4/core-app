...
export default function CreateTeamInvoice({
  supabase,
  teamId,
  wallet,
  connection,
  setRefetch,
}: CreateTeamInvoiceProps) {
  ...
  const onSubmit = form.handleSubmit(async (formData) => {
    ...
    try {
      ...
      const instruction = await createTeamInvoiceInstruction({
        wallet,
        connection,
      ...
      toast.dismiss(toastId);
      toast.error("Failed to create team invoice");
      setLoading(false); // Ensure loading is reset if the operation cannot proceed
      return;
    }
  });
